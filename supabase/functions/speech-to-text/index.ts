...
Deno.serve(async (req) => {
  try {
    ...
    const transcriptionResponse = await openai.audio.transcriptions.create({
      ...
      model: 'whisper-1',
      language: 'en',
      prompt: 'The user is describing what they want to happen next in a story.',
    })
    ...
    if (storyContext && finalTranscript) {
      finalTranscript = await correctTranscriptWithContext(finalTranscript, storyContext, openai)
    }
